...
This meta-skill provides guidelines for creating, modifying, and maintaining Codex skills and hooks for this project.
...
```bash
mkdir -p .Codex/skills/[skill-name]/resources
```
...
| `Stop` | When Codex stops responding |
...
```json
{
  "hooks": {
    "HookType": [
      {
        "type": "shell",
        "command": "bash .Codex/hooks/hook-name.sh",
...
Output printed to stdout is displayed to Codex.

## Creating Slash Commands

### Command Structure

Create `.Codex/commands/[command-name].md`:

```markdown
# Command Title

[Instructions for Codex when this command is invoked]
...
### Commands
1. **Clear instructions** - Tell Codex exactly what to do
